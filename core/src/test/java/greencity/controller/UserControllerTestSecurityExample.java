...
class UserControllerTestSecurityExample {
    ...
    @Test
    void shouldReturn200WhenAdminAccessesPrivate() throws Exception {
        mockMvc.perform(get("/user/privateTest")
    ...
    @Test
    void shouldReturn403WhenUserAccessesPrivate() throws Exception {
        mockMvc.perform(get("/user/privateTest", 1L)
    ...
    void shouldReturn200ForPublicEndpoint() throws Exception {
        mockMvc.perform(get("/user/publicTest")
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk());
    }
